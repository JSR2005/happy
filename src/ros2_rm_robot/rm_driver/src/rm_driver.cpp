...
void RmArm::Arm_Set_Force_Postion_new_Callback(const rm_ros_interfaces::msg::Setforcepositionnew::SharedPtr msg)
{
    RCLCPP_INFO(this->get_logger(), "收到力位混合指令:sensor=%d, mode=%d", msg->sensor, msg->mode);
    rm_force_position_t p{};
    p.sensor = msg->sensor;
    p.mode   = msg->mode;

    for (int i = 0; i < 6; ++i) {
        p.control_mode[i]  = msg->control_mode[i];
        p.desired_force[i] = msg->desired_force[i];
        p.limit_vel[i]     = msg->limit_vel[i];
    }

    // handle_ 需要是你 rm_driver 里已经连接好的机械臂句柄
    int ret = rm_set_force_position_new(robot_handle, p);

    std_msgs::msg::Bool ok;
    ok.data = (ret == 0);
    Set_Force_Postion_new_Result->publish(ok);

    RCLCPP_INFO(this->get_logger(),"rm_set_force_position_new ret=%d (0 ok, -1/-2/-3 fail)",ret);
}
...
/**********************************************************************透传力位混合控制***********************************************************/
    ...
    Set_Force_Postion_Cmd = this->create_subscription<rm_ros_interfaces::msg::Setforceposition>("rm_driver/set_force_postion_cmd",rclcpp::ParametersQoS(),
        ...
        sub_opt4);
    Set_Force_Postion_new_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/set_force_postion_new_result", rclcpp::ParametersQoS());
    Set_Force_Postion_new_Cmd = this->create_subscription<rm_ros_interfaces::msg::Setforcepositionnew>("rm_driver/set_force_postion_new_cmd",rclcpp::ParametersQoS(),
        std::bind(&RmArm::Arm_Set_Force_Postion_new_Callback,this,std::placeholders::_1),
        sub_opt4);  
    /********************************************************结束力位混合控制*******************************************************************/
